...
class DeepRL:
	...
	def createNet(self):
		W_conv1 = self.weight_variable([filter1_dim, filter1_dim, frames, filter1_depth])
		b_conv1 = self.bias_variable([filter1_depth]) 

		W_conv2 = self.weight_variable([filter2_dim, filter2_dim, filter1_depth, filter2_depth])
		b_conv2 = self.bias_variable([filter2_depth]) 

		W_conv3 = self.weight_variable([filter3_dim, filter3_dim, filter2_depth, filter3_depth])
		b_conv3 = self.bias_variable([filter3_depth]) 

		W_fc1 = self.weight_variable([7*7*filter3_depth, neurons])
		b_fc1 = self.bias_variable([neurons])

		W_fc2 = self.weight_variable([neurons, actions])
		...
		# hidden layers
		h_conv1 = tf.nn.relu(self.conv2d(s, W_conv1, filter1_stride) + b_conv1)
		h_conv2 = tf.nn.relu(self.conv2d(h_conv1, W_conv2, filter2_stride) + b_conv2) 
		h_conv3 = tf.nn.relu(self.conv2d(h_conv2, W_conv3, filter3_stride) + b_conv3) 
